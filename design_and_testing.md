...
### The market gap
Imagine a Chief Marketing Officer at a B2B firm asking: "We spent $150,000 on the 'Offshore Wind' campaign over the last 12 months. Did it actually help us win the $10M Equinor contract?"
...
### The team gap
Historically, extracting cross-channel insights required dedicated data analysts writing SQL queries, or marketers spending weeks mastering BI tools. This creates an operational bottleneck between seeing the data and deciding on the next best action.
...
- **The CMO lobby view:** An executive dashboard providing marketing effectiveness metrics (e.g., pipeline velocity, blended CPA) without requiring deep dives into individual tactics.
...
An architectural decision was made to isolate all LLM interactions within a dedicated service layer (`app/services/llm_rotator.py`). 
...
I chose Python and FastAPI for the backend. Python is well-suited for data analytics, while FastAPI provides excellent performance and native asynchronous support for non-blocking I/O—which is essential for handling LLM API calls and database queries concurrently.

### Frontend: HTMX + Alpine.js + Jinja2
To avoid the overhead of a heavy SPA framework, I used HTMX to swap HTML fragments directly into the DOM. This keeps state management firmly on the server, acting as a reliable delivery mechanism for the programmatic AI responses. Alpine.js handles lightweight client-side interactivity (like tab switching and Chart.js initialization), and Jinja2 dynamically generates the HTML.

### Styling: Tailwind CSS
Tailwind enabled rapid UI prototyping directly within the HTML templates, allowing for quick iterations on the dashboard's design without managing complex external stylesheets.

### Database: SQLite
SQLite is highly portable and requires zero configuration, making it ideal for the prototype's embedded data simulation. It allowed for rapid iteration on the database schema. For a production deployment, this would be swapped out for PostgreSQL to support concurrent writes and data persistency.
...
Traditional deterministic testing is insufficient for evaluating the usefulness of LLM-generated insights. An output can be mathematically correct but contextually useless.
- **Approach**: The `scripts/ai_evals_matrix.py` script runs a cross-product matrix test, executing MCP tools across various campaigns and feeding the output into a secondary Gemini LLM "Judge".
- **Measurement criteria**: The Judge evaluates the payload on Actionability and Contextual Relevance on a 1-5 scale. 
- **Iterative refinement**: This harness drove the development process. Initially, many tools scored 1/5 because they returned raw data blobs. Based on the evals, I enriched the Python logic—for instance, `map_buying_committee` now explicitly returns `job_titles` and segmentation data. The `eval_reports/` directory tracks these improvements.
